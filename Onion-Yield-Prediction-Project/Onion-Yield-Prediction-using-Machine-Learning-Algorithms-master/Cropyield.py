...
main = tkinter.Tk()
main.title("Onion Yield Prediction using RNN, Feedforward and LSTM Neural Network")
...
def upload():
    ...
    onion_dataset = pd.read_csv('dataset/Agriculture In India.csv')
    onion_dataset.fillna(0, inplace = True)
    onion_dataset['Production'] = onion_dataset['Production'].astype(np.int64)
    print(onion_dataset.dtypes)
    print(onion_dataset['Production'])
    text.delete('1.0', END)
    text.insert(END,filename+' Loaded\n\n')
    text.insert(END,str(onion_dataset.head))
...
def preprocess():
    ...
    text.delete('1.0', END)
    le = LabelEncoder()
    onion_dataset['State_Name'] = pd.Series(le.fit_transform(onion_dataset['State_Name']))
    onion_dataset['District_Name'] = pd.Series(le.fit_transform(onion_dataset['District_Name']))
    onion_dataset['Season'] = pd.Series(le.fit_transform(onion_dataset['Season']))
    onion_dataset['Onion'] = pd.Series(le.fit_transform(onion_dataset['Onion']))
    onion_datasets = onion_dataset.values
    cols = onion_datasets.shape[1]-1
    X = onion_datasets[:,0:cols]
    Y = onion_datasets[:,cols]
    Y = Y.astype('uint8')
    avg = np.average(Y)
    #avg = avg / 60
    Y1 = []
...
def predict():
    ...
    test['Onion'] = pd.Series(le.fit_transform(test['Onion']))
    test = test.values
    cols = test.shape[1]
    test = test[:,0:cols]
    ...
    for i in range(len(test)):
        ...
        print(str(predict))
        if predict == 0:
            text.insert(END,"X=%s, Predicted = %s" % (test[i], 'Predicted Onion Yield will be LESS')+"\n\n")
        else:
            text.insert(END,"X=%s, Predicted = %s" % (test[i], 'Predicted Onion Yield will be HIGH')+"\n\n")
...
def topGraph():
    ...
    rice = rice.groupby(['State_Name','Onion'])['Production'].agg(['sum'])
    ...
    fig.suptitle('Top 6 State Rainfall & Onion Yield')
    ax[0].plot(x1,y1.copy())
    ax[0].set_title("State Vs Rainfall")
    ax[1].plot(x2,y2.copy())
    ax[1].set_title("Top 6 State Vs Rice Onion Yield")
    ax[2].plot(x3,y3.copy())
    ax[2].set_title("Top 6 State Vs Coconut Onion Yield")
    ax[3].plot(x4,y4.copy())
    ax[3].set_title("Top 6 State Vs Sugarcane Onion Yield")
    ax[4].plot(x5,y5.copy())
    ax[4].set_title("Top 6 State Vs Any Onion Yield")
    plt.show()        

    
   
font = ('times', 15, 'bold')
title = Label(main, text='Onion Yield Prediction using RNN, Feedforward and LSTM Neural Network', justify=LEFT)
...
topButton = Button(main, text="Top 6 Onion Yield Graph", command=topGraph)
